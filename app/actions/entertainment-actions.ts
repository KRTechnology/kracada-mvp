...
// new actions for creating custom entertainment posts
 ...
 */
export async function createEntertainmentPostAction(data: CreatePostData) {
  try {
    ...
    if (!isContributor) {
      return {
        success: false,
        message: "Only Contributors can create entertainment posts",
      };
    }
    ...
    const existingPost = await db
      ...
      .where(eq(entertainmentPosts.slug, validatedData.slug))
      .limit(1);
    ...
    revalidatePath("/entertainment/videos");
...
/**
 * Update a entertainment post
 */
export async function updateEntertainmentPostAction(data: UpdatePostData) {
  try {
    const user = await getCurrentUser();
    const userId = user.id as string;
    ...
    const [existingPost] = await db
      ...
      .where(
        and(
          eq(entertainmentPosts.id, validatedData.id),
          eq(entertainmentPosts.authorId, userId),
        ),
      )
      .limit(1);
    ...
    const [updatedPost] = await db
      .update(entertainmentPosts)
      .set(updateData)
      .where(eq(entertainmentPosts.id, validatedData.id))
      .returning();

    revalidatePath("/entertainment");
    revalidatePath("/entertainment/videos");
    revalidatePath(`/entertainment/${validatedData.id}`);
...
/**
 * Delete a entertainment post
 */
export async function deleteEntertainmentPostAction(postId: string) {
  try {
    const user = await getCurrentUser();
    const userId = user.id as string;
    ...
    const [existingPost] = await db
      ...
      .where(
        and(
          eq(entertainmentPosts.id, postId),
          eq(entertainmentPosts.authorId, userId),
        ),
      )
      .limit(1);
...
/**
 * Get entertainment posts with pagination and filtering
 */
export async function getEntertainmentPostsAction(params?: {
  page?: number;
  limit?: number;
  authorId?: string;
  ...
  try {
    ...
    if (params?.authorId) {
      conditions.push(eq(entertainmentPosts.authorId, params.authorId));
    }

    if (params?.status) {
      conditions.push(eq(entertainmentPosts.status, params.status));
    } else {
      // By default, only show published posts (unless filtering by specific status)
      conditions.push(eq(entertainmentPosts.status, "published"));
    }

    // Query posts with author info
    const posts = await db
      .select({
        id: entertainmentPosts.id,
        title: entertainmentPosts.title,
        slug: entertainmentPosts.slug,
        description: entertainmentPosts.description,
        content: entertainmentPosts.content,
        featuredImage: entertainmentPosts.featuredImage,
        featuredImageKey: entertainmentPosts.featuredImageKey,
        categories: entertainmentPosts.categories,
        status: entertainmentPosts.status,
        authorId: entertainmentPosts.authorId,
        viewCount: entertainmentPosts.viewCount,
        likeCount: entertainmentPosts.likeCount,
        commentCount: entertainmentPosts.commentCount,
        publishedAt: entertainmentPosts.publishedAt,
        createdAt: entertainmentPosts.createdAt,
        updatedAt: entertainmentPosts.updatedAt,
        author: {
          id: users.id,
      ...
      .from(entertainmentPosts)
      .leftJoin(users, eq(entertainmentPosts.authorId, users.id))
      .where(conditions.length > 0 ? and(...conditions) : undefined)
      .orderBy(
        desc(entertainmentPosts.publishedAt),
        desc(entertainmentPosts.createdAt),
      )
      .limit(limit)
      .offset(offset);
...
/**
 * Get a single entertainment post by ID or slug
 */
export async function getEntertainmentPostAction(identifier: string) {
  try {
    ...
    const [post] = await db
      ...
      .where(
        or(
          eq(entertainmentPosts.id, identifier),
          eq(entertainmentPosts.slug, identifier),
        ),
      )
      .limit(1);
    ...
    await db
      .update(entertainmentPosts)
      .set({ viewCount: sql`${entertainmentPosts.viewCount} + 1` })
      .where(eq(entertainmentPosts.id, post.id));
...
/**
 * Toggle like on a entertainment post
 */
export async function toggleEntertainmentPostLikeAction(postId: string) {
  try {
    const user = await getCurrentUser();
    const userId = user.id as string;
    ...
    const existingLike = await db
      ...
      .where(
        and(
          eq(entertainmentPostLikes.postId, postId),
          eq(entertainmentPostLikes.userId, userId),
        ),
      )
      .limit(1);
    ...
    if (existingLike.length > 0) {
      ...
      await db
        .update(entertainmentPosts)
        .set({ likeCount: sql`${entertainmentPosts.likeCount} - 1` })
        .where(eq(entertainmentPosts.id, postId));
      ...
      // Like: Add the like
      await db.insert(entertainmentPostLikes).values({
        postId,
        userId,
      });
...
/**
 * Create a comment on a entertainment post
 */
export async function createEntertainmentCommentAction(
  ...
  try {
    ...
    await db
      .update(entertainmentPosts)
      .set({ commentCount: sql`${entertainmentPosts.commentCount} + 1` })
      .where(eq(entertainmentPosts.id, validatedData.postId));

    revalidatePath(`/entertainment/${validatedData.postId}`);
...
/**
 * Get comments for a entertainment post
 */
export async function getEntertainmentCommentsAction(postId: string) {
  try {
    const comments = await db
      .select()
      .from(entertainmentComments)
      .where(eq(entertainmentComments.postId, postId))
      .orderBy(desc(entertainmentComments.createdAt));
...
export async function getAuthorStatsAndPostsAction(authorId: string) {
  try {
    ...
    const authorPosts = await db
      ...
      .where(
        and(
          eq(entertainmentPosts.authorId, authorId),
          eq(entertainmentPosts.status, "published"),
        ),
      )
      .orderBy(desc(entertainmentPosts.publishedAt))
      .limit(5);
