...
import { updateHotelAction } from "@/app/(dashboard)/actions/hotels-restaurants-actions";
import {
...
export function EditHotelForm({
  ...
  const removeGalleryImage = (index: number) => {
    ...
    // Only remove from files if it's a new upload (has a data: URL)
    ...
    try {
      ...
      if (galleryImageFiles.length > 0) {
        ...
        for (const file of galleryImageFiles) {
          ...
          galleryFormData.append("hotelId", hotel.id);
      ...
      const result = await updateHotelAction(hotel.id, hotelData);

      if (result.success) {
        toast.success("Hotel updated successfully!");
        router.push("/dashboard/hotels-restaurants");
      } else {
        toast.error(result.error || "Failed to update hotel");
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      {/* Form - Reusing structure from AddHotelForm but with pre-filled data */}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input id="checkIn" {...register("checkIn")} />
              {errors.checkIn && (
                <p className="text-sm text-red-500 mt-1">
            ...
            <div>
              ...
              <Input id="checkOut" {...register("checkOut")} />
              {errors.checkOut && (
                <p className="text-sm text-red-500 mt-1">
            ...
            <div>
              ...
              <Input id="smoking" {...register("smoking")} />
              {errors.smoking && (
                <p className="text-sm text-red-500 mt-1">
