...
import {
  getNewsCommentsAction,
  createNewsCommentAction,
  toggleNewsPostLikeAction,
  checkNewsPostLikedAction,
} from "@/app/(dashboard)/actions/news-actions";

interface NewsArticleFooterProps {
  postId: string;
}

export const NewsArticleFooter = ({ postId }: NewsArticleFooterProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Fetch comments
      const commentsResult = await getNewsCommentsAction(postId);
      ...
      if (session?.user?.id) {
        const likeResult = await checkNewsPostLikedAction(postId);
  ...
  const handleLike = async () => {
    ...
    try {
      const result = await toggleNewsPostLikeAction(postId);
      if (result.success) {
